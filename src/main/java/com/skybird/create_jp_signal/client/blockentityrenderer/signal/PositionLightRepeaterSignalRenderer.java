...
public class PositionLightRepeaterSignalRenderer implements ISignalHeadRenderer {
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay, SignalHead headData, BlockEntity blockEntity , int packedLight, int packedOverlay, Vec3 offset, Pair<Double, Double> rotation) {
        ...
        if (VisualizationManager.supportsVisualization(blockEntity.getLevel())) {
            ...
            msr.rotateCentered(Axis.YP.rotationDegrees((float)(double)rotation.getFirst())).translate(offset.x, offset.y, offset.z);
            ...
            switch (accessory) {
                case FORECAST -> {
                    ...
                    msr.translate(-7.0/16, (2.25-8.0)/16, 1.75/16);
                    msr.scale(3.5f);
                    for (int i = 0; i < 2; i++) {
                        LampColor color = colors.next();
                        modelRenderer.renderModel(
                ...
                case INDICATOR_HOME -> {
                    ms.pushPose();
                    Iterator<LampColor> colors = SignalAccessory.getLampColors(accessory, route).iterator();
                    msr.translate(-5.0/16, 0.75/16 - 1, 1.75/16);
                    msr.scale(2.5f);
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 3; j++) {
                            LampColor color = colors.next();
                ...
                case INDICATOR_DEPARTURE -> {
                    ms.pushPose();
                    Iterator<LampColor> colors = SignalAccessory.getLampColors(accessory, route).iterator();
                    msr.translate(0, (0.75 - 11.0)/16, 1.75/16);
                    msr.scale(2.5f);
                    LampColor color = colors.next();
                    modelRenderer.renderModel(
                        ms.last(),
                ...
                case INDICATOR_SHUNT -> {
                    ...
                    {
                        ms.pushPose();
                        msr.translate(-3.75/16, 0, 0);
                        msr.scale(1.5f, 3f, 1f);
                        for (int i = 0; i < 3; i++) {
                            LampColor color = colors.next();
                            modelRenderer.renderModel(
                                ms.last(),
